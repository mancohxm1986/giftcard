...
if ( !class_exists( 'PL_WCPT_List_Table' ) ) {
	
	class PL_WCPT_List_Table {
	...
	<?php
			foreach ( $months as $arc_row ) {
				...
				printf( "<option %s value='%s'>%s</option>\n",
					...
					sprintf( esc_attr__( '%1$s %2$d' ), esc_attr( $wp_locale->get_month( $month ) ), esc_attr( $year ) )
				);
			}
	?>
			</select>
	...
	<?php
		...
		protected function comments_bubble( $post_id, $pending_comments ) {
			...
			echo wp_kses_post( "<a href='" . esc_url( add_query_arg( 'p', $post_id, admin_url( 'edit-comments.php' ) ) ) . "' title='" . esc_attr( $pending_phrase ) . "' class='post-com-count'><span class='comment-count'>" . number_format_i18n( get_comments_number() ) . "</span></a>" );
		...
		public function print_column_headers( $with_id = true ) {
			...
			foreach ( $columns as $column_key => $column_display_name ) {
				...
				echo wp_kses_post( "<th scope='col' " . esc_attr( $id ) . " " . wp_kses_post( $class ) . " " . wp_kses_post( $style ) . ">" . wp_kses_post( $column_display_name ) . "</th>" );
			}
		}
